...
void shooteru_snddev_poweramp_on(int en)
{
	...
	if (en) {
		msleep(50);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_HP_EN), 1);
		set_speaker_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_SPEAKER;
	} else {
		set_speaker_amp(0);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
...
void shooteru_snddev_imic_pamp_on(int en)
{
	...
	pr_info("%s %d\n", __func__, en);
	shooteru_snddev_bmic_pamp_on(en);
	if (en) {
...
void shooteru_snddev_bmic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_MIC_SEL), 0);
	} else {
...
void shooteru_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_MIC_SEL), 1);
	}
}
...
void shooteru_snddev_fmhs_pamp_on(int en)
{
	pr_info("%s %d\n", __func__, en);
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_HP_EN), 1);
		set_headset_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_FM_HS;
	} else {
		set_headset_amp(0);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_FM_HS;
	}
}
...
int shooteru_support_adie(void)
{
	return 0;
}
...
int shooteru_support_beats(void)
{
...
void shooteru_enable_beats(int en)
{
	pr_info("%s: %d\n", __func__, en);
	set_beats_on(en);
}
...
void shooteru_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int shooteru_get_q6_effect_mode(void)
{
...
static struct q6v2audio_icodec_ops iops = {
	...
	.support_adie = shooteru_support_adie,
...
static struct acoustic_ops acoustic = {
	...
	.support_beats = shooteru_support_beats,
	.enable_beats = shooteru_enable_beats,
	.set_q6_effect = shooteru_set_q6_effect_mode,
};
...
void shooteru_audio_gpios_init(void)
{
	pr_info("%s\n", __func__);
	gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_HP_EN), "AUD_SPK_ENO");
	gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTERU_AUD_MIC_SEL), "AUD_HANDSET_ENO");
}
