...
struct mdp_table_entry shooteru_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
int shooteru_mdp_gamma_cool(void)
{
...
static struct msm_panel_common_pdata mdp_pdata = {
	...
	.mdp_gamma_cool = shooteru_mdp_gamma_cool,
};
...
#ifdef CONFIG_FB_MSM_HDMI_MHL
static mhl_driving_params shooteru_driving_params[] = {
...
#ifdef CONFIG_FB_MSM_HDMI_MHL
	.driving_params =  shooteru_driving_params,
	.driving_params_count = ARRAY_SIZE(shooteru_driving_params),
#endif
};
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		v_lcm = regulator_get(NULL,
				lcm_str);
		if (IS_ERR(v_lcm)) {
			pr_err("[DISP] could not get %s, rc = %ld\n",
		...
		if (panel_type == PANEL_ID_SHR_SHARP_NT) {
			...
			if (rc) {
				pr_err("[DISP] %s#%d: set_voltage %s failed, rc=%d\n", __func__, __LINE__, lcm_str, rc);
				return -EINVAL;
			}
		}
		...
		if (rc) {
			pr_err("[DISP] %s:LCM gpio %d request failed, rc=%d\n", __func__,  GPIO_LCM_RST_N, rc);
			return -EINVAL;
		}
		dsi_power_on = true;
	}
	...
	if (on) {
		...
		if (rc < 0) {
			pr_err("[DISP] set_optimum_mode %s failed, rc=%d\n", lcm_str, rc);
			return -EINVAL;
		}
		hr_msleep(1);
		...
		if (rc) {
			pr_err("[DISP] enable regulator %s failed, rc=%d\n", lcmio_str, rc);
			return -ENODEV;
		}
		...
		if (regulator_disable(v_lcm)) {
			pr_err("[DISP] %s: Unable to enable the regulator: %s\n", __func__, lcm_str);
			return -EINVAL;
		}
		hr_msleep(5);
...
static struct platform_device mipi_dsi_shooteru_panel_device = {
	.name = "mipi_shooteru",
	.id = 0,
};
...
int mipi_cmd_shooteru_qhd_pt_init(void);

void __init shooteru_init_fb(void)
{
	msm8x60_set_display_params("mipi_shooteru", "hdmi_msm");
	platform_device_register(&msm_fb_device);
	platform_device_register(&mipi_dsi_shooteru_panel_device);
