...
static int mipi_shooteru_send_cmd(struct dsi_cmd_desc *cmd, unsigned int len, bool clk_ctrl)
{
	int ret = 0;
...
static struct msm_panel_common_pdata *mipi_shooteru_pdata;

static int mipi_shooteru_lcd_init(void);
...
void mipi_shooteru_panel_type_detect(void)
{
...
static int mipi_shooteru_display_on(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
  unsigned int val = 0;
  ...
  mipi_shooteru_send_cmd(novatek_display_on_cmds, ARRAY_SIZE(novatek_display_on_cmds), true);
...
static int mipi_shooteru_lcd_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
...
static int mipi_shooteru_display_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;

  mfd = platform_get_drvdata(pdev);

  mipi_shooteru_send_cmd(mipi_power_off_cmd, mipi_power_off_cmd_size, true);
...
static int mipi_shooteru_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_shooteru_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static struct platform_driver this_driver = {
	.probe  = mipi_shooteru_lcd_probe,
	.driver = {
		.name   = "mipi_shooteru",
	},
};

static struct msm_fb_panel_data shooteru_panel_data = {
	.on		= mipi_shooteru_lcd_on,
        .off		= mipi_shooteru_lcd_off,
        .set_backlight  = mipi_shooteru_set_backlight,
	.late_init = mipi_shooteru_display_on,
	.early_off = mipi_shooteru_display_off,
};
					...
					u32 channel, u32 panel)
{
	...
	ret = mipi_shooteru_lcd_init();
	if (ret) {
		pr_err("mipi_shooteru_lcd_init() failed with ret %u\n", ret);
		return ret;
	}

	pdev = platform_device_alloc("mipi_shooteru", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	shooteru_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &shooteru_panel_data,
		sizeof(shooteru_panel_data));
	if (ret) {
		printk(KERN_ERR
...
static int mipi_shooteru_lcd_init(void)
{
        mipi_shooteru_panel_type_detect();
