...
static uint32_t mhl_usb_switch_ouput_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_USB_SWITCH, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
static void shooteru_usb_dpdn_switch(int path)
{
	...
	case PATH_MHL:
	{
		...
		gpio_set_value(SHOOTERU_GPIO_MHL_USB_SWITCH, (mhl ^ !polarity) ? 1 : 0);
		break;
	}
	}
...
static int64_t shooteru_get_usbid_adc(void)
{
	int mean_adc;
	int adc[5];
	int i, ret = 0;
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.get_adc_cb	= shooteru_get_usbid_adc,
...
static uint32_t mhl_gpio_table[] = {
	GPIO_CFG(SHOOTERU_GPIO_MHL_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(SHOOTERU_GPIO_MHL_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
};
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	.gpio_intr = SHOOTERU_GPIO_MHL_INT,
	.gpio_reset = SHOOTERU_GPIO_MHL_RESET,
	.ci2ca = 0,
...
static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(SHOOTERU_CAM_I2C_SDA, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(SHOOTERU_CAM_I2C_SCL, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi4_gpio_table_gpio[] = {
	GPIO_CFG(SHOOTERU_CAM_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_CAM_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi5_gpio_table[] = {
	GPIO_CFG(SHOOTERU_TP_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(SHOOTERU_TP_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi5_gpio_table_gpio[] = {
	GPIO_CFG(SHOOTERU_TP_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_TP_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table_gpio[] = {
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_GENERAL_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi10_gpio_table[] = {
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi10_gpio_table_gpio[] = {
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SENSOR_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static int shooteru_ts_cy8c_set_rst(int on)
{
	...
	struct pm8058_gpio_cfg tp_rst[] = {
		{ /* TW RST Set LOW */
			PM8058_GPIO_PM_TO_SYS(SHOOTERU_TP_RST),
			{
...
static int shooteru_ts_cy8c_power(int on)
{
	printk(KERN_INFO "%s():\n", __func__);
	if (on)
		shooteru_ts_cy8c_set_rst(1);

	return 0;
}

static int shooteru_ts_cy8c_reset(void)
{
	printk(KERN_INFO "[TP] HW reset touch.\n");
	shooteru_ts_cy8c_set_rst(0);
	msleep(10);
	shooteru_ts_cy8c_set_rst(1);
	msleep(200);
...
struct cy8c_i2c_platform_data shooteru_ts_cy8c_data[] = {
	{
		.version = 0x0C,
		.timeout = 1,
		.unlock_attr = 1,
		...
		.abs_width_min = 0,
		.abs_width_max = 512,
		.power = shooteru_ts_cy8c_power,
		.gpio_irq = SHOOTERU_TP_ATT_N_XB,
		/*.filter_level = {40, 80, 942, 982},*/
		.reset = shooteru_ts_cy8c_reset,
	},
...
static void shooteru_ts_cy8c_set_system_rev(uint8_t rev)
{
	ssize_t i = 0;

	if (rev >= PVT_VERSION)
		for (i = 0; i < sizeof(shooteru_ts_cy8c_data)/sizeof(struct cy8c_i2c_platform_data); i++)
			shooteru_ts_cy8c_data[i].auto_reset = 1;
}

static struct i2c_board_info msm_i2c_gsbi5_info[] = {
	{
		I2C_BOARD_INFO(CYPRESS_TMA_NAME, 0x67),
		.platform_data = &shooteru_ts_cy8c_data,
		.irq = MSM_GPIO_TO_INT(SHOOTERU_TP_ATT_N_XB)
	},
};
...
static struct platform_device rpm_regulator_early_device __devinitdata = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &shooteru_rpm_regulator_early_pdata,
	},
};
...
static struct platform_device rpm_regulator_device __devinitdata = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &shooteru_rpm_regulator_pdata,
	},
};
...
static struct platform_device *shooteru_devices[] __initdata = {
...
static void __init shooteru_early_memory(void)
{
	reserve_info = &msm8x60_reserve_info;
}

static void __init shooteru_reserve(void)
{
	msm_reserve();
}
...
static uint32_t msm_spi_gpio[] = {
	GPIO_CFG(SHOOTERU_SPI_DO,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SPI_DI,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SPI_CS,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(SHOOTERU_SPI_CLK, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = SHOOTERU_AUD_HP_EN,
...
static struct isl29028_platform_data isl29028_pdata = {
	.intr = PM8058_GPIO_PM_TO_SYS(SHOOTERU_PLS_INT),
...
static struct i2c_board_info i2c_isl29028_devices[] = {
	{
		...
		.irq = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, SHOOTERU_PLS_INT),
	},
};
...
#endif
#ifdef CONFIG_SERIAL_MSM_HS_BRCM
	msm_uart_dm1_pdata.wakeup_irq = gpio_to_irq(SHOOTERU_GPIO_BT_HOST_WAKE);
...
static void __init shooteru_init(void)
{
	...
	platform_add_devices(shooteru_devices,
			     ARRAY_SIZE(shooteru_devices));

        shooteru_gpio_mpp_init();
	...
	if (properties_kobj) {
          rc = sysfs_create_group(properties_kobj, &shooteru_properties_attr_group);
	}
...
#endif
	shooteru_ts_cy8c_set_system_rev(system_rev);
...
static void __init shooteru_fixup(struct tag *tags,
